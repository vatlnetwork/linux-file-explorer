...
class TagsService extends ChangeNotifier {
  ...
  Future<void> _loadTags() async {
    ...
    if (availableTagsJson != null) {
      try {
        ...
        _availableTags =
            availableTagsList.map((tagJson) => Tag.fromMap(tagJson)).toList();
      } catch (e) {
  ...
  List<Tag> getTagsForFile(String filePath) {
    ...
    return _availableTags.where((tag) => tagIds.contains(tag.id)).toList();
  }
  ...
  Future<void> deleteTag(String tagId) async {
    ...
    _fileTags.forEach((path, tagIds) {
      tagIds.remove(tagId);
    });

    // Remove empty file entries
    _fileTags.removeWhere((_, tagIds) => tagIds.isEmpty);
  ...
  Future<void> renameTag(String tagId, String newName) async {
    final tagIndex = _availableTags.indexWhere((tag) => tag.id == tagId);
    if (tagIndex != -1) {
      final tag = _availableTags[tagIndex];
      _availableTags[tagIndex] = Tag(
        id: tag.id,
        name: newName,
        color: tag.color,
      );
