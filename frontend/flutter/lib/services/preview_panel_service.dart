...
class PreviewPanelService extends ChangeNotifier {
  ...
  List<QuickAction> getQuickActionsFor(FileItem item) {
    // Return empty list to remove all quick actions
    return [];
  }
  ...
  Future<String?> getTextFileContent(String path, {int maxLines = 200}) async {
    try {
      ...
      if (size > 1024 * 1024) {
        // Don't try to read files larger than 1MB
  ...
  Future<List<FileItem>> getDirectoryContent(
    String path, {
    int maxItems = 100,
  }) async {
    try {
  ...
  Future<void> savePreviewOptions(
  ...
  Future<void> savePreviewOptionsForItem(
    PreviewOptions options,
    FileItem item,
  ) async {
    if (item.type == FileItemType.directory) {
    ...
    try {
      ...
      showDialog(
        ...
        builder:
            (context) => AlertDialog(
              title: Text(
                'Compressing ${_selectedItem!.type == FileItemType.directory ? 'Folder' : 'File'}...',
              ),
              content: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
      ...
      final originalSize = await compressionService.getUncompressedSize(
        _selectedItem!.path,
      );

      // Compress the file or folder
      final outputPath = await compressionService.compressToZip(
        _selectedItem!.path,
      );

      // Get compressed size
      final compressedSize = await compressionService.getCompressedSize(
        outputPath,
      );
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Column(
            ...
            children: [
              Text(
                '${_selectedItem!.type == FileItemType.directory ? 'Folder' : 'File'} compressed to ${p.basename(outputPath)}',
              ),
              Text(
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Failed to compress ${_selectedItem!.type == FileItemType.directory ? 'folder' : 'file'}: $e',
          ),
  ...
  void _handleDuplicate(BuildContext context) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Failed to duplicate file: $e')));
    }
  }
  ...
  void handleRotate(BuildContext context) async {
    ...
    if (!['.jpg', '.jpeg', '.png', '.gif', '.bmp', '.webp'].contains(ext)) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Rotation is only supported for image files'),
        ),
      );
      return;
    }
    ...
    try {
      ...
      if (result.exitCode != 0) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: const Text(
              'ImageMagick is not installed. Please install it to use image rotation.',
            ),
              ...
              onPressed: () async {
                try {
                  ...
                  if (context.mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text('ImageMagick installed successfully'),
                      ),
                    );
                  }
                } catch (e) {
                  if (context.mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text('Failed to install ImageMagick: $e'),
                      ),
                    );
                  }
                }
            ...
            (context) => AlertDialog(
              content: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 16),
                  Text(
                    'Rotating image ${degrees > 0 ? 'right' : 'left'} ${degrees.abs()}°...',
                  ),
                ],
              ),
            ),
        ...
        try {
          ...
          if (context.mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text(
                  'Image rotated ${degrees > 0 ? 'right' : 'left'} ${degrees.abs()}°',
                ),
                action: SnackBarAction(
                  label: 'Undo',
                  onPressed: () async {
                    try {
                      ...
                      if (context.mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text('Failed to undo rotation: $e'),
                          ),
                        );
                      }
                    }
  ...
  void _handleCreatePdf(BuildContext context) async {
    ...
    if (![
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'PDF creation is currently only supported for text files',
          ),
        ),
      );
      return;
    ...
    try {
      ...
      final outputPath =
          '${_selectedItem!.path.substring(0, _selectedItem!.path.lastIndexOf('.'))}.pdf';
    ...
    try {
      ...
      if (ffmpegResult.exitCode != 0 || tesseractResult.exitCode != 0) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'FFmpeg and Tesseract OCR are required for searchable PDF creation.',
            ),
          ),
        );
        return;
      ...
      final outputPath =
          '${_selectedItem!.path.substring(0, _selectedItem!.path.lastIndexOf('.'))}_searchable.pdf';
      ...
      if (ocrResult.exitCode == 0) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Searchable PDF created: ${p.basename(outputPath)}'),
          ),
        );
      } else {
  ...
  void _handleCreateAlias(BuildContext context) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Failed to create alias: $e')));
    }
  }
  ...
  void _handleAddToFavorites(BuildContext context) async {
    ...
    try {
      final favoritesDir = Directory(
        '${Platform.environment['HOME']}/.favorites',
      );
      ...
      if (result.exitCode == 0) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Added to favorites: ${p.basename(aliasPath)}'),
          ),
        );
      } else {
        throw Exception(result.stderr);
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Failed to add to favorites: $e')));
    }
  }
    ...
    try {
      ...
      if (result.exitCode != 0) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Tesseract OCR is not installed. Please install it to use text extraction.',
            ),
          ),
        );
        return;
      ...
      final outputPath =
          '${_selectedItem!.path.substring(0, _selectedItem!.path.lastIndexOf('.'))}.txt';
      final ocrResult = await Process.run('tesseract', [
        _selectedItem!.path,
      ...
      if (ocrResult.exitCode == 0) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Text extracted to: ${p.basename(outputPath)}'),
          ),
        );
      } else {
        throw Exception(ocrResult.stderr);
  ...
  void _handleRevealInFolder(BuildContext context) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Failed to reveal in folder: $e')));
    }
  }
    ...
    try {
      ...
      if (result.exitCode != 0) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'ffmpeg is not installed. Please install it to use audio conversion features.',
            ),
                  ...
                  onTap: () async {
                    ...
                    try {
                      ...
                      if (context.mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text('Failed to convert audio: $e'),
                          ),
                        );
                      }
                    }
                      ...
                      if (result.exitCode == 0) {
                        if (context.mounted) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                              content: Text(
                                'Audio converted to FLAC successfully',
                              ),
                            ),
                          );
                        }
    ...
    try {
      ...
      if (result.exitCode != 0) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'FFmpeg is not installed. Please install it to use video compression.',
            ),
            duration: Duration(seconds: 5),
          ),
        );
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('Error compressing video: $e')));
      }
    }
  }
  ...
  Future<void> _compressVideo(BuildContext context, String quality) async {
    ...
    try {
      ...
      final outputPath =
          '${_selectedItem!.path.substring(0, _selectedItem!.path.lastIndexOf('.'))}_compressed.mp4';
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('Failed to compress video: $e')));
      }
    }
  }
      ...
      if (!success) {
        try {
          ...
          if (fehResult.exitCode == 0) {
            final result = await Process.run('feh', [
              '--bg-scale',
              _selectedItem!.path,
            ]);
            if (result.exitCode == 0) {
    ...
    try {
      ...
      if (result.exitCode != 0) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'FFmpeg is not installed. Please install it to use audio extraction.',
            ),
          ),
        );
        return;
      ...
      final outputPath =
          '${_selectedItem!.path.substring(0, _selectedItem!.path.lastIndexOf('.'))}.wav';
      ...
      if (extractResult.exitCode == 0) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Audio extracted to: ${p.basename(outputPath)}'),
          ),
        );
      } else {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Failed to extract audio: $e')));
    }
  }
}
